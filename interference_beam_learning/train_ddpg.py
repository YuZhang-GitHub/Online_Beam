...
def train(ch, options, train_options, beam_id):
    ...
    bf_gain_t_file = 'pfs/bf_gain_t_record_beam_' + str(beam_id) + '.txt'
    interf_1_file = 'pfs/interf_1_record_beam_' + str(beam_id) + '.txt'
    interf_2_file = 'pfs/interf_2_record_beam_' + str(beam_id) + '.txt'
    sir_file = 'pfs/sir_record_beam_' + str(beam_id) + '.txt'
    precision = '%.5f'
    ...
    with torch.cuda.device(options['gpu_idx']):
        ...
        while iteration < num_of_iter:
            ...
            critic_net.eval()
            q_pred = critic_net(state, action_quant)
            ...
            # store: train session recording
            if train_options['overall_iter'] % options['save_record'] == 0:
                if os.path.exists(bf_gain_t_file):
                    bf_t = torch.Tensor.cpu(CB_Env.bf_gain_t_ach.detach()).numpy().reshape(1, -1)
                    with open(bf_gain_t_file, 'ab') as f:
                        np.savetxt(f, bf_t, fmt=precision, delimiter='\n')
                else:
                    bf_t = torch.Tensor.cpu(CB_Env.bf_gain_t_ach.detach()).numpy().reshape(1, -1)
                    np.savetxt(bf_gain_t_file, bf_t, fmt=precision, delimiter='\n')

                if os.path.exists(interf_1_file):
                    i1 = torch.Tensor.cpu(CB_Env.interf_gain_1_ach.detach()).numpy().reshape(1, -1)
                    with open(interf_1_file, 'ab') as f:
                        np.savetxt(f, i1, fmt=precision, delimiter='\n')
                else:
                    i1 = torch.Tensor.cpu(CB_Env.interf_gain_1_ach.detach()).numpy().reshape(1, -1)
                    np.savetxt(interf_1_file, i1, fmt=precision, delimiter='\n')

                if os.path.exists(interf_2_file):
                    i2 = torch.Tensor.cpu(CB_Env.interf_gain_2_ach.detach()).numpy().reshape(1, -1)
                    with open(interf_2_file, 'ab') as f:
                        np.savetxt(f, i2, fmt=precision, delimiter='\n')
                else:
                    i2 = torch.Tensor.cpu(CB_Env.interf_gain_2_ach.detach()).numpy().reshape(1, -1)
                    np.savetxt(interf_2_file, i2, fmt=precision, delimiter='\n')

                if os.path.exists(sir_file):
                    sir = torch.Tensor.cpu(CB_Env.threshold_sir.detach()).numpy().reshape(1, -1)
                    with open(sir_file, 'ab') as f:
                        np.savetxt(f, sir, fmt=precision, delimiter='\n')
                else:
                    sir = torch.Tensor.cpu(CB_Env.threshold_sir.detach()).numpy().reshape(1, -1)
                    np.savetxt(sir_file, sir, fmt=precision, delimiter='\n')

            sir = torch.Tensor.cpu(CB_Env.threshold_sir.detach()).numpy().reshape(1, -1)

            print(
                "Beam: %d, Iteration: %d, Q value: %.4f, Reward: %d, BF Gain pred: %.2f, BF Gain: %.2f, Critic Loss: %.2f, Policy Loss: %.2f, Best SIR: %.2f." % \
